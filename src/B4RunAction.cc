...
G4double neutronMaxEnergyLim=0.10*eV; //100 meV
    // Creating histograms // name and description of istograms
    analysisManager->CreateH1("ENeutron","Neutron Energy in Detector",300, 0.001*eV, 1*eV,"eV","none","log"); // third entry is type of binning (log binning)//nb min can't be 0 in log scale//nb da anche il max dell'istogramma
    ...
    analysisManager->CreateH2("EBvsZB","Energy vs Z on boundary", 300, -1.50*m, 1.51*m, 100, 0.0005e-04*eV, neutronMaxEnergyLim, "m", "eV", "none", "none", "linear", "linear");
