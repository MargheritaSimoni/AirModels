...
public:
    ...
    // Updated: now TrackID is passed to count each Ar41/C14 track only once
    void AddAr41(G4int trackID, G4double Ar41E, G4ThreeVector Ar41Position);

    //%%%%%%%%%%%%%%%%%%%%%% Secondary particles analysis %%%%%%%%%%%%%%%%%%%%%%%%%%
    void AddSecondaryParticle(
            G4int trackid,
            G4int Z,
            G4int A,
            G4double secondaryE,
            G4double secondaryTrackLengths
    );
    ...
    */
    ...
    // New getters: return all recorded energies/positions for Ar41
    std::vector<G4double> GetEAr41Vector() const;

    std::vector<G4ThreeVector> GetAr41PositionVector() const;
...
private:
    ...
    // Ar41 single-value variables kept for compatibility
    G4double fAr41;
    G4double eAr41;
    G4ThreeVector ArPos;


    // New logic: store already-counted TrackIDs
    std::set<G4int> fCountedAr41TrackIDs;

    // New logic: store one energy/position entry per unique particle track
    std::vector<G4double> eAr41Vector;

    std::vector<G4ThreeVector> Ar41PosVector;
...
inline G4int B4cCalorHit::GetNAr41() const
{
    return int(fAr41);
}

inline G4double B4cCalorHit::GetEAr41() const
{
    return eAr41;
}

inline G4ThreeVector B4cCalorHit::GetAr41Position() const
{
    return ArPos;
}


// New getters: vectors of all unique Ar41 and C14 tracks

inline std::vector<G4double> B4cCalorHit::GetEAr41Vector() const
{
    return eAr41Vector;
}


inline std::vector<G4ThreeVector> B4cCalorHit::GetAr41PositionVector() const
{
    return Ar41PosVector;
}

//%%%%%%%%%%%%%%%%%%%%%% Secondary particles analysis %%%%%%%%%%%%%%%%%%%%%%%%%%

inline std::vector<G4int> B4cCalorHit::GetAtomicNumber() const
{
    return fZ;
}

inline std::vector<G4int> B4cCalorHit::GetAtomicMass() const
{
    return fA;
}

inline std::vector<G4double> B4cCalorHit::GetSecondaryEnergy() const
{
    return eSecondary;
}
